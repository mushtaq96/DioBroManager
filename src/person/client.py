...
client = SyncFHIRClient(url='https://fhir.nasfjq8u71h8.static-test-account.isccloud.io', extra_headers={"x-api-key":"1gQBs8CG2F1uPodxxUF0T7s5SwOWhqAZ4LGwZPAs"})
...
#Create a new patient using fhir.resources
    ...
    name = HumanName()
    name.use = "official"
    name.family = person['family_name'] #"familyname"
    name.given = [person['given_name']] #["givenname1","givenname2"]
    ...
    status = client.resource('Patient',**json.loads(patient0.json())).save()
...
#Get the patient as a fhir.resources Patient of our list of patient resources who has the right name, for convenience we will use the patient we created before
    patient0 = Patient.parse_obj(patients_resources.search(family=familyName).first().serialize())
    return patient0
